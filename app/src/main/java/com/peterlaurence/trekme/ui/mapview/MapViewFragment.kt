        ...
        if (projection != null) {
            launch {
                ...
                if (projectedValues != null) {
                    if(i==0)
                    {
                        Log.d("Borhan 1 ", "message"+projectedValues[0]+"  "+projectedValues[1]+" "+location.latitude+"  "+location.longitude)
                        updatePosition(projectedValues[0], projectedValues[1])
                        i=1
                    }else
                    {
                        Log.d("Borhan 2 ", "message"+projectedValues[0]+"  "+projectedValues[1]+" "+location.latitude+"  "+location.longitude)
                        updatePosition(location.longitude-0.00001, location.latitude-0.00001)
                        i=0
                    }
            ...
            Timer().schedule(object : TimerTask() {
                override fun run() {
                    Log.d("Borhan", "message")
                    updatePosition(location.longitude+i, location.latitude)
                    i=i+0.001
                }
            }, 2000)*/
            Log.d("Borhan 3", "message"+location.latitude+"  "+location.longitude)
